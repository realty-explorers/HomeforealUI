...
export default class DealsController {
	...
	public findDeals = async (
		...
		try {
			const { regionId, distance, profit, soldMinPrice, soldMaxPrice, daysOnZillow } = req.body;
			...
			const response = await this.dealsService.getDeals(regionId, distance, profit, soldMinPrice, soldMaxPrice, daysOnZillow);
			res.json(response);
		} catch (error: any) {
			console.log(error);
