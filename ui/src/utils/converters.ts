...
const priceScale = (value: number) => {
  ...
  for (const level of levels) {
    const newLevelConsecutiveSteps = levelConsecutiveSteps +
      (level.max - levelConsecutiveMax) / level.step;
    if (value < newLevelConsecutiveSteps) {
      price = levelConsecutiveMax +
        (value - levelConsecutiveSteps) * level.step;
      break;
    }
